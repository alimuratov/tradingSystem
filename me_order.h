...
namespace Exchange {
    struct MEOrder {
        ...
        MEOrder() = default;
        MEOrder(TickerId ticker_id, ClientId client_id, OrderId client_order_id, OrderId market_order_id, Side side, Price price, Qty qty, Priority priority, MEOrder* prev_order, MEOrder* next_order)
    ...
    struct MEOrdersAtPrice {
        ...
        auto toString() const -> std::string {
            ...
            ss << "MEOrdersAtPrice: {"
               ...
               << "first_me_order: " << (first_me_order_ ? first_me_order_->toString() : "nullptr") << ", "
